...
public class HttpHelper {
    ...
    private HttpHelper() {
        ...
        userAgents
                .add("Mozilla/5.0 (Windows NT 6.1; WOW64; rv:22.0) Gecko/20100101 Firefox/22.0"); // FireFox
        userAgents
                .add("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.1453.110 Safari/537.36"); // Chrome
        userAgents
                .add("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/534.57.2 (KHTML, like Gecko) Version/5.1.7 Safari/534.57.2"); // Safari
        userAgents
                .add("Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0)"); // IE9
        req.header("User-Agent",
                userAgents.get(new Random().nextInt(userAgents.size()))); // 随机
        res = new Response();
    }
    ...
    private static ByteBuffer readToByteBuffer(InputStream inStream, int maxSize)
            throws IOException {
    ...
    public HttpHelper charset(String charset) {
        Validate.isTrue(Charset.isSupported(charset),
                String.format("The charset of '%s' is not supported", charset));
        req.charset(charset);
        return this;
    }
    ...
    public static class Response extends Base<Response> {
        ...
        private static void serialiseRequestUrl(Request req)
                throws IOException {
            URL in = req.url();
        ...
        public String html() throws IOException {
            ...
            if (charset == null) {
                ...
                content = Charset.forName(defaultCharset).decode(byteData)
                        .toString();

                Pattern pattern = Pattern
                        .compile("<meta[^>]*content=\"(.+?)\"[^>]*>");
                ...
                if (foundCharset != null) {
                    ...
                    content = Charset.forName(foundCharset).decode(byteData)
                            .toString();
                }
            } else {
